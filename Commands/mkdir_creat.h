...
int enter_name(MINODE *pmip, int ino, char *name)
{
	...
	for(dblock = 0; dblock < 12; dblock++)					//execute across all direct blocks within inode's inode table
	{
		if(!pmip->inode.i_block[dblock])				//break if empty block found
			break;
		...
		if(remain >= need_len)						//if remaining space is greater than the required space then insert
		{
			...
			put_block(pmip->dev, pmip->inode.i_block[dblock], buf);	//write data block to device
			iput(pmip);
	...
	if(dblock < 12 && pmip->inode.i_block[dblock] == 0)			//if empty block found then create a new data block
	{
